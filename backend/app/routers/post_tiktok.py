...
from ..schemas.post_tiktok import (
    PostTikTokCreate, PostTikTokUpdate, PostTikTokOut
)

router = APIRouter(prefix="/posts-tiktok", tags=["posts-tiktok"])

@router.post("/", response_model=PostTikTokOut, status_code=201)
def create_post_tiktok(payload: PostTikTokCreate, db: Session = Depends(get_db)):
...
def get_post_tiktok(asset_id: int, db: Session = Depends(get_db)):
    asset = get_asset(db, asset_id)
    if not asset or asset.tipo_asset != TipoAssetEnum.post_tiktok:
        raise HTTPException(status_code=404, detail="Post do TikTok não encontrado")
    return asset

@router.put("/{asset_id}", response_model=PostTikTokOut)
def update_post_tiktok(asset_id: int, payload: PostTikTokUpdate, db: Session = Depends(get_db)):
    asset = get_asset(db, asset_id)
    if not asset or asset.tipo_asset != TipoAssetEnum.post_tiktok:
        raise HTTPException(status_code=404, detail="Post do TikTok não encontrado")
...
def delete_post_tiktok(asset_id: int, db: Session = Depends(get_db)):
    asset = get_asset(db, asset_id)
    if not asset or asset.tipo_asset != TipoAssetEnum.post_tiktok:
        raise HTTPException(status_code=404, detail="Post do TikTok não encontrado")
